...
from app.config.settings import WSAA_WSDL, WSFE_WSDL, ENVIRONMENT
    ...
    try:
        ...
        if ENVIRONMENT == "dev" and ("MOCKTOKEN" in ticket_data.get("token", "") or "Failed to create SOAP client" in str(e)):
            ...
            def mock_fe_cae_solicitar(**kwargs):
                cbte_desde = kwargs.get("FeCAEReq", {}).get("FeDetReq", {}).get("FECAEDetRequest", [{}])[0].get("CbteDesde", 1)
                return SimpleNamespace(
                    FeCabResp=SimpleNamespace(
                        Resultado="A",
                        CantReg=1,
                        PtoVta=kwargs.get("FeCAEReq", {}).get("FeCabReq", {}).get("PtoVta", 1),
                        CbteTipo=kwargs.get("FeCAEReq", {}).get("FeCabReq", {}).get("CbteTipo", 1),
        ...
        try:
            if used_fallback:
                ...
                try:
                    ...
                    resultado = cab_resp.find('Resultado', {}).text if cab_resp and cab_resp.find('Resultado', {}) is not None else "R"
                    pto_vta = int(cab_resp.find('PtoVta', {}).text) if cab_resp and cab_resp.find('PtoVta', {}) is not None else 0
                    cbte_tipo = int(cab_resp.find('CbteTipo', {}).text) if cab_resp and cab_resp.find('CbteTipo', {}) is not None else 0
                    fch_proceso = cab_resp.find('FchProceso', {}).text if cab_resp and cab_resp.find('FchProceso', {}) is not None else ""
                    cant_reg = int(cab_resp.find('CantReg', {}).text) if cab_resp and cab_resp.find('CantReg', {}) is not None else 0
                    reproceso = cab_resp.find('Reproceso', {}).text if cab_resp and cab_resp.find('Reproceso', {}) is not None else "N"
                    
                    # Extract detail data
                    det_resp = result_elem.find('.//FECAEDetResponse', {})
                    concepto = int(det_resp.find('Concepto', {}).text) if det_resp and det_resp.find('Concepto', {}) is not None else 0
                    doc_tipo = int(det_resp.find('DocTipo', {}).text) if det_resp and det_resp.find('DocTipo', {}) is not None else 0
                    doc_nro = int(det_resp.find('DocNro', {}).text) if det_resp and det_resp.find('DocNro', {}) is not None else 0
                    cbte_desde = int(det_resp.find('CbteDesde', {}).text) if det_resp and det_resp.find('CbteDesde', {}) is not None else 0
                    cbte_hasta = int(det_resp.find('CbteHasta', {}).text) if det_resp and det_resp.find('CbteHasta', {}) is not None else 0
                    cbte_fch = det_resp.find('CbteFch', {}).text if det_resp and det_resp.find('CbteFch', {}) is not None else ""
                    cae = det_resp.find('CAE', {}).text if det_resp and det_resp.find('CAE', {}) is not None else ""
                    cae_fch_vto = det_resp.find('CAEFchVto', {}).text if det_resp and det_resp.find('CAEFchVto', {}) is not None else ""
                    resultado_det = det_resp.find('Resultado', {}).text if det_resp and det_resp.find('Resultado', {}) is not None else "R"
                ...
                if hasattr(client.transport, 'session') and '_original_post' in locals():
                    client.transport.session.post = _original_post
